...
def order_csv_filesyt(directory, descending=False):
    # get the current directory
...
##
def append_df_nodate(directory_y, csv_files):
    #CSV ->>> Dataframe, columns++
    print(directory_y)
    print(len(csv_files))
    df_comments = pd.DataFrame()
    #for i in range(len(csv_files)):
       # if 3==3:                 #os.path.isfile(os.path.join(directory_y, "{}.csv".format(csv_files[i]))):
    data = pd.read_csv(
                os.path.join(directory_y, "{}".format(csv_files[0])),
                sep=",",
                engine="python",
                on_bad_lines="skip",
                usecols=[
        ...
        #else : print('Jij bent gek')
    df_comments = pd.concat([df_comments, data], ignore_index=True)
    return df_comments
...
def retrieve_app_id(appname): #alleen voor play scraper
    file = path.join("Data_playstore/playstore", "name_id_map.csv")
    df = pd.read_csv(file)
    id = None
    try:
